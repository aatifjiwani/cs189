    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        for h in range(Hout):
            for wi in range(Wout):
                toConvolute = padded_x[:, h*self.stride : h*self.stride+HH, wi*self.stride : wi*self.stride+WW, :]
                for f in range(F):
                    Z[:, h, wi, f] = np.sum(toConvolute*WEIGHT[:, :, :, f], axis=(1,2,3)) + b[0, f]
