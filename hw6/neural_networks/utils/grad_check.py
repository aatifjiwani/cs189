...
def check_gradients(
    ...
    while not it.finished:
        ...
        # handle special case of backpropagation through time for RNN,
        # where chain rule must be applied across timesteps as well.
        if has_time:
        ...
        it.iternext()
    print('num grad: {}'.format(numgrad))

    return norm(numgrad - grad) / norm(numgrad + grad) + 1e-10
